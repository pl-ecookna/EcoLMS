...
- в Dokploy: адрес того API, который доступен из контейнера `web`
  (обычно `http://api:3001`, если сервисы находятся в одной внутренней сети).

Если меняется адрес backend, достаточно обновить только эту переменную.
