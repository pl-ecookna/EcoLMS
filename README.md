...
- при запуске обработки встречи UI показывает ожидаемое время выполнения на основе истории завершённых записей и размера файла;
...
- фактическое время обработки встречи считается по `processing_started_at -> processing_finished_at`, а прогноз строится по завершённым встречам схожего размера.
