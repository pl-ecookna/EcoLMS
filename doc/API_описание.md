...
- `GET /api/health`
  Возвращает сводку по `api`, `postgres`, `redis`, `llm`, `speechProvider`, `worker`, `transcriptionService`, а также счётчики проектов и upload-сессий. Активный STT-провайдер дополнительно передаётся как `speechProviderName`.
